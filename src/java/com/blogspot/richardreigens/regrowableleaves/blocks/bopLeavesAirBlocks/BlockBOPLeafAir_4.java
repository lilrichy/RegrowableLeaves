...
public class BlockBOPLeafAir_4 extends BlockAir {
    public static final PropertyEnum<BOPPage_4.EnumType> TYPE = PropertyEnum.create("type", BOPPage_4.EnumType.class);
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        if (!worldIn.isRemote) {
            if (rand.nextInt(10) > ConfigurationHandler.generalSettings.leafRegrowthRate && worldIn.getLight(pos) >= ConfigurationHandler.generalSettings.lightRequiredToGrow) {
                if (ConfigurationHandler.generalSettings.debugMode) LogHelper.info("BlockBOPLeafAir_4 Tick");
