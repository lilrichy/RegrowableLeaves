...
public class BlockBOPLeafAir_2 extends BlockAir {
    public static final PropertyEnum<BOPPage_2.EnumType> TYPE = PropertyEnum.create("type", BOPPage_2.EnumType.class);
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        if (!worldIn.isRemote) {
            if (rand.nextInt(10) > ConfigurationHandler.generalSettings.leafRegrowthRate && worldIn.getLight(pos) >= ConfigurationHandler.generalSettings.lightRequiredToGrow) {
                if (ConfigurationHandler.generalSettings.debugMode) LogHelper.info("BlockBOPLeafAir_2 Tick");
