...
public class BlockBOPLeafAir_0 extends BlockAir {
    public static final PropertyEnum TYPE = PropertyEnum.create("type", BOPPage_0.EnumType.class);
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        if (!worldIn.isRemote) {
            if (rand.nextInt(10) > ConfigurationHandler.leafRegrowthRate && worldIn.getLight(pos) >= ConfigurationHandler.lightRequiredToGrow) {
                if (ConfigurationHandler.debugMode) LogHelper.info("BlockBOPLeafAir_0 Tick");
